...
27. 谈谈 keep-alive
  - keepalive 可以实现对组件的缓存，当组件切换时，不会对当前组件进行卸载，常用的2个属性 include/exclude，2个生命周期 activated/deactivated
  - LRU算法：最近最久未使用法
  ```js
  // 实现LRU算法？
  ```
28. vue性能优化
  - 不要将所有数据放在data中，特别是渲染列表，Object.freeze 冻结数据
  - spa页面采用keepalive
  - 拆分组件
  - 懒加载和异步组件
  - 数据持久化，防抖，节流
...
36. Vue.set原理
  -
37. vue compile 过程？指令/插值表达式等vue语法如何实现
  -
